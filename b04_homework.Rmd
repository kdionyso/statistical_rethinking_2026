...
par(mfrow = c(5, 1))
models <- list(mPPO = postPPO, mFE = postMFE, mML = postmML, mML_nc_corr = postmML_nc_pp_corr, mLMMCF = postLMMCF)
for (mod in names(models)) {
    visualize_data(post_model = models[[mod]], model_label = mod)
}
```

Now, normalized proportions by district average.
```{r echo=FALSE}
...
```
Or across all districts for all models
```{r out.height="90%", fig.cap="Effect of having more kids on contraception use. Effects are reported on having one additional child."}
dat_counter0 <- dat
dat_counter1 <- dat
par(mfrow = c(4, 1))

for (l in 0:3) {
    plot(NULL, xlim = c(-1, 0.8), ylim = c(0, 20), xlab = "p[C]", ylab = "Density", main = paste0("Difference between ", l, " and ", l + 1, " kids."))
    ...
    for (mod in names(models)) {
        ...
        # postLMMCF <- extract.samples(models[])
        # dat_counter0$K <- rep(l, length(dat$K))
        # dat_counter1$K <- rep(l + 1, length(dat$K))
        # fake_list0 <- dat_counter0
        # fake_list1 <- dat_counter1
        ...
        p0 <- sapply(1:length(fake_list0$K), function(i) {
            inv_logit(
                models[[mod]]$a[, fake_list0$D[i]] +
                    # bA * fake_list0$A +
                    # bU * U +
                    models[[mod]]$bK * sum(models[[mod]]$delta_j[, 1:fake_list0$K[i]])
                # bUG * UG[fake_list0$D]))
            )
        })
        ...
        p1 <- sapply(1:length(fake_list1$K), function(i) {
            inv_logit(
                models[[mod]]$a[, fake_list1$D[i]] +
                    # bA * fake_list0$A +
                    # bU * U +
                    models[[mod]]$bK * sum(models[[mod]]$delta_j[, 1:fake_list1$K[i]])
                # bUG * UG[fake_list0$D]))
            )
        })

        # for (i in 1:max(dat$D)) {
        dens(p0 - p1, add = TRUE, col = c("black", "blue", 4, 2)[color_ind], lw = 4)
        # }
    }
}
```
